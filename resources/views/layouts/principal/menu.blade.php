@foreach (Session::get('packageDinamicMenuChocoprintsPaquetes') as $paquete)
    <!-- LINEA DE DIVISION -->
    <hr class="sidebar-divider">
    {{-- PAQUETE - NIVEL 1 DEL MENU --}}
    <div class="sidebar-heading">{{ $paquete->nombrePaquete }}</div>
    @foreach (Session::get('packageDinamicMenuChocoprintsModulos')[$paquete->idPaquete] as $modulo)
        <li class="nav-item">
            ...
            <div id="collapse{{ $modulo->idModulo }}" class="collapse" aria-labelledby="heading{{ $modulo->idModulo }}"
                ...
                <div class="bg-white py-2 collapse-inner rounded">
                    @foreach (Session::get('packageDinamicMenuChocoprintsOpciones')[$modulo->idModulo] as $opcion)
